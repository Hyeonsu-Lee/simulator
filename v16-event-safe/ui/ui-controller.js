...
class UIController {
    ...
    async waitForCharacterData() {
        ...
        let attempts = 0;
        while (!characterLoader.loaded && attempts < 50) {
            await new Promise(resolve => setTimeout(resolve, 100));
            attempts++;
        }
        
        if (!characterLoader.loaded) {
            console.warn('Character data not loaded, using built-in specs');
            characterLoader.loadBuiltInSpecs();
        }
    }
    ...
    setupEventListeners() {
        ...
        this.elements.distanceSelect.addEventListener('change', (e) => 
            configManager.set('simulation.distance', parseInt(e.target.value))
        );
        
        this.elements.coreSize.addEventListener('change', (e) => 
            configManager.set('simulation.coreSize', parseInt(e.target.value))
        );
        
        this.elements.cubeSelect.addEventListener('change', (e) => 
            configManager.set('simulation.cubeType', e.target.value)
        );
        
        this.elements.eliteCode.addEventListener('change', (e) => 
            configManager.set('simulation.eliteCode', e.target.value)
        );
        
        this.elements.simSpeed.addEventListener('change', (e) => 
            configManager.set('simulation.speed', parseInt(e.target.value))
        );
        
        this.elements.runCount.addEventListener('change', (e) => 
            configManager.set('simulation.runCount', parseInt(e.target.value))
        );
    }
     ...
     */
    subscribeToEvents() {
        eventBus.on(Events.UI_UPDATE, (event) => this.updateStats(event.data.stats));
        eventBus.on(Events.LOG_MESSAGE, (event) => this.updateLog(event.data));
        eventBus.on(Events.PROGRESS_UPDATE, (event) => this.updateProgress(event.data));
        eventBus.on(Events.SIMULATION_COMPLETE, (event) => this.showResults(event.data));
    }
    ...
    createSquadUI() {
        ...
        // 캐릭터 옵션 추가
        characterLoader.getAllIds().forEach(id => {
            const spec = characterLoader.getSpec(id);
            for (let i = 0; i < 5; i++) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            select.addEventListener('change', (e) => {
                ...
                configManager.setSquadMember(i, characterId || null);
                
                if (characterId) {
                    configDiv.classList.remove('hidden');
                    
                    levelInput.onchange = () => {
                        configManager.setCharacterConfig(characterId, {
        ...
        document.getElementById('targetSelect').addEventListener('change', (e) => {
            configManager.setTargetIndex(parseInt(e.target.value));
        });
    }
    ...
    updateOverloadConfig() {
        ...
        selects.forEach(select => {
            ...
            if (select.value && level) {
                configManager.setOverloadOption(equipment, slot, {
                    type: select.value,
                    level: parseInt(level)
                });
            } else {
                configManager.setOverloadOption(equipment, slot, null);
            }
        });
    }
    ...
    updateStats(stats) {
        ...
        this.elements.stats.totalDamage.textContent = formatNumber(stats.totalDamage);
        this.elements.stats.currentDPS.textContent = formatNumber(stats.dps);
    ...
    updateLog(logEntry) {
        ...
        this.elements.logTitle.textContent = 
            `전투 로그 (최근 50개 표시, 전체 ${logger.logs.length}개 기록됨)`;
    }
