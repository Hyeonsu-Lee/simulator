...
class BuffSystem {
    ...
    subscribeToEvents() {
        eventBus.on(Events.BUFF_APPLY, (event) => this.handleBuffApply(event));
        eventBus.on(Events.BUFF_REMOVE, (event) => this.handleBuffRemove(event));
        eventBus.on(Events.BUFF_UPDATE, (event) => this.handleBuffUpdate(event));
    ...
    handleBuffApply(event) {
        ...
        if (existingBuff && stackable) {
            ...
            existingBuff.refreshTime(event.time);
        } else {
            // 새 버프 생성
            const buff = new Buff({
        ...
        charBuffs.forEach(buff => {
            ...
            Object.entries(stats).forEach(([stat, value]) => {
                if (stat === 'fixedATK' && value.source) {
                    // fixedATK 특수 처리
                    const sourceCharSpec = characterLoader.getSpec(value.source);
                    if (sourceCharSpec) {
    ...
    updateCharacterBuffState(characterId) {
        ...
        stateStore.set(`buffs.active.${characterId}`, {
            buffs: buffs,
     ...
     * 버프 변환
     */
    transformBuff(targetId, fromBuffId, toBuffId, newStats) {
        const charBuffs = this.activeBuffs.get(targetId);
        if (!charBuffs) return;
